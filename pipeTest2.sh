...
build/cloneHD --cna $set_n/data/$tu_cna --baf $set_n/data/$tu_baf --snv $set_n/data/$tu_snv --pre $set_n/results/$tu_cna --bias $set_n/support/$norm_cna.posterior-1.txt --seed 123 --trials 5 --nmax 4 --max-tcn $set_n/data/chr_ploidy_hardH2pO.txt --cna-jumps $set_n/support/$tu_cna.bias.jumps.txt --baf-jumps $set_n/support/$tu_baf.jumps.txt --min-jump 0.1 --restarts 5 --mass-gauging 0
